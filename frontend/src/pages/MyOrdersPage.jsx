  ...
  useEffect(() => {
    const mockOrders = [
      ...
      }, 
      {
        _id: "67890",
        createdAt: new Date(),
        shippingAddress: { city: "Hanoi", address: "456 Street" },
        orderItems: [
          {
            name: "Bánh bao",
    ...
    // Nếu bạn muốn delay 1 giây trước khi set data
        ...
        <table className='min-w-full text-left text-gary-500'>
          ...
          <tbody>
            {orders.length > 0 ? (
              orders.map((order) => (
                ...
                className='border-b hover:bg-gray-50 cursor-pointer'>
                  <td className='py-2 px-2 sm:py-4 sm:px-4'>
                    <img src={order.orderItems[0].image} alt={order.orderItems[0].name} className='w-10 h-10 sm:h-12 sm:w-12 object-cover rounded' />
                  </td>
                  {/* id sản phẩm  */}
                  ...
                  {/* địa chỉ giao hàng */}
                  <td className='py-2 px-2 sm:py-4 sm:px-4'>
                    {`${order.shippingAddress ?` ${order.shippingAddress.address}, ${order.shippingAddress.city}` : 'Chưa có địa chỉ'}`}
                    </td>
                  {/* sản phẩm */}
                  <td className='py-2 px-2 sm:py-4 sm:px-4'>
                  ...
                  <td className='py-2 px-2 sm:py-4 sm:px-4'>
                    <span 
                     className={`${order.isPaid ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'} px-2 py-1 rounded-full text-xs sm:text-sm font-medium`}
                    >
              ...
              <tr>
                <td colSpan="8" className='py-4 px-4 text-center text-[#db2777]'>Không có đơn hàng nào</td>
              </tr>  
            )}
          </tbody>
        </table>
